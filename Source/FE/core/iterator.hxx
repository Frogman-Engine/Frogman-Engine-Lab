...
{
	using category = contiguous_iterator<typename std::remove_const<T>::type>;
	using value_type = T;
...
public:
	...
	_FORCE_INLINE_ pointer operator+(difference_type pointer_offset_p) const noexcept
	{
		FE_ASSERT(this->m_iterator == nullptr, "${%s@0}: The iterator was null.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_NULLPTR));
		return this->m_iterator + pointer_offset_p;
	}

	_FORCE_INLINE_ void operator+=(difference_type pointer_offset_p) noexcept
	{
		FE_ASSERT(this->m_iterator == nullptr, "${%s@0}: The iterator was null.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_NULLPTR));
	...
	_FORCE_INLINE_ void operator-=(const contiguous_iterator& value_p) noexcept
	{
		FE_ASSERT(this->m_iterator == nullptr, "${%s@0}: The iterator was null.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_NULLPTR));
		this->m_iterator -= value_p.m_iterator;
	}
...
public:
	...
	_FORCE_INLINE_ void operator-=(const obsessive_iterator& value_p) noexcept
	{
		FE_ASSERT(this->m_iterator._smart_ptr == nullptr, "${%s@0}: The iterator was null.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_NULLPTR));
		this->m_iterator.operator-=(value_p.m_iterator);
	}
	...
	struct ptr
	{
		...
		_FORCE_INLINE_ T* operator+(ptrdiff_t pointer_offset_p) const noexcept
		{
			FE_ASSERT(this->_smart_ptr == nullptr, "${%s@0}: The iterator was null.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_NULLPTR));
			return this->operator->() + pointer_offset_p;
		}

		_FORCE_INLINE_ void operator+=(ptrdiff_t pointer_offset_p) noexcept
		{
			FE_ASSERT(this->_smart_ptr == nullptr, "${%s@0}: The iterator was null.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_NULLPTR));
		...
		_FORCE_INLINE_ T& operator[](index_t index_p) const noexcept
		{
			FE_ASSERT(this->_smart_ptr == nullptr, "${%s@0}: The iterator was null.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_NULLPTR));
			FE_ASSERT(index_p == FE::max_value<index_t>, "${%s@0}: negative integers cannot be assigned to an index_t variable.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_RANGE));
