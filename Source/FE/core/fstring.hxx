...
public:
    ...
    _CONSTEXPR20_ fixed_sized_string(const value_type* const string_p) noexcept : m_fstring{ _NULL_ }, m_length(algorithm::string::length<CharT>(string_p))
    {
        ...
        FE_SUSPECT(this->m_length >= Capacity, "${%s@0}: the input string length exceeds the max string capacity.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_CAPACITY));
    ...
    {
        FE_SUSPECT(index_p >= this->m_length, "${%s@0}: index out of boundary.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_RANGE));
    ...
    {
        FE_SUSPECT(count_p >= Capacity, "${%s@0}: ${%s@1} exceeds fixed sized string capacity.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_CAPACITY), TO_STRING(count_p));
    ...
    _CONSTEXPR20_ fixed_sized_string& assign(const value_type* const string_p, const size_type size_to_assign_p) noexcept
    {
        ...
        FE_SUSPECT(string_p == nullptr, "${%s@0}: {%s@1} is nullptr.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_NULLPTR), TO_STRING(string_p));
        FE_SUSPECT(size_to_assign_p >= Capacity, "ERROR: size_to_assign_p exceeds fixed sized string capacity.");
    ...
    _CONSTEXPR20_ fixed_sized_string& assign(const_iterator input_begin_p, const_iterator input_end_p) noexcept
    {
        ...
        FE_SUSPECT((input_end_p - input_begin_p) >= Capacity, "${%s@0}: input string range length exceeds fixed sized string capacity.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_RANGE));
    ...
    {
        FE_SUSPECT(initializer_list_p.size() >= Capacity, "ERROR: initializer_list_p.size() exceeds the string capacity.");
    ...
    {
        FE_SUSPECT(position_p >= Capacity, "ERROR: position_p cannot be greater than the string capacity.");
        FE_SUSPECT(count_p == 0, "ERROR: insert() operation was not successful. size_type count_p was zero.");
        FE_SUSPECT((this->m_length + count_p) >= Capacity, "ERROR: fixed sized string capacity overflowed.");
    ...
    {
        FE_SUSPECT(position_p >= Capacity, "${%s@0}: position_p cannot be greater than the string capacity.", TO_STRING(FE::MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_RANGE));
    ...
    {
        FE_SUSPECT(position_p >= Capacity, "${%s@0}: ${%s@1} cannot be greater than the ${%s@2}.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_CAPACITY), TO_STRING(position_p), TO_STRING(Capacity));
        FE_SUSPECT((this->m_length + other_p.m_length) >= Capacity, "${%s@0}: fixed sized string capacity overflowed.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_CAPACITY));
    ...
    _CONSTEXPR20_ fixed_sized_string& insert(const size_type position_p, std::initializer_list<const CharT>&& initializer_list_p) noexcept
    {
        ...
        FE_SUSPECT((this->m_length + initializer_list_p.size()) >= Capacity, "${%s@0}: fixed sized string capacity overflowed.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_CAPACITY));
    ...
    {
        FE_SUSPECT(Capacity < (index_p + count_p), "${%s@0}: fixed sized string capacity overflowed.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_RANGE));
    ...
    {
        FE_SUSPECT(this->m_length >= Capacity, "${%s0}: out of capacity", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_RANGE));
    ...
    _CONSTEXPR20_ fixed_sized_string& append(const size_type input_count_p, const value_type value_p) noexcept
    {
        ...
        FE_SUSPECT(this->max_length() < (this->m_length + input_count_p), "${%s0}: cannot append ${%ld@1} character(s) to the fstring.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_RANGE), &input_count_p);
    ...
    _CONSTEXPR20_ fixed_sized_string& append(const_iterator input_begin_p, const_iterator input_end_p) noexcept
    {
        ...
        FE_SUSPECT((input_end_p - input_begin_p) + this->m_length >= Capacity, "${%s@0}: input string range length exceeds fixed sized string capacity.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_RANGE));
    ...
    _CONSTEXPR20_ fixed_sized_string& append(std::initializer_list<const CharT>&& initializer_list_p) noexcept
    {
        ...
        FE_SUSPECT(this->max_length() < initializer_list_p.size() + this->m_length, "${%s@0}: initializer_list_p is too large to take the contents.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_CAPACITY));
    ...
    {
        FE_SUSPECT(((this->m_length + other_p.m_length) - count_to_replace_p) > this->max_length(), "${%s@0}: failed to replace.", TO_STRING(FE::MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_CAPACITY));
    ...
    {
        FE_SUSPECT(((this->m_length + other_count_p) - count_to_replace_p) > this->max_length(), "${%s@0}: failed to replace.", TO_STRING(FE::MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_CAPACITY));
        FE_SUSPECT(position_p > this->m_length, "${%s@0}: ${%s@1} must not be greater than ${%s@2}.", TO_STRING(FE::MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_RANGE), TO_STRING(position_p), TO_STRING(this->m_length));
    ...
    _CONSTEXPR20_ fixed_sized_string& replace(const size_type position_p, const size_type count_to_replace_p, const value_type* const string_p, const size_type input_count_p) noexcept
    {
        ...
        FE_SUSPECT(((this->m_length + input_count_p) - count_to_replace_p) > this->max_length(), "${%s@0}: failed to replace.", TO_STRING(FE::MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_CAPACITY));
        FE_SUSPECT(count_to_replace_p == 0, "${%s@0}: ${%s@1} is zero.", TO_STRING(FE::MEMORY_ERROR_1XX::_FATAL_ERROR_INVALID_SIZE), TO_STRING(count_to_replace_p));
        FE_SUSPECT(string_p == nullptr, "${%s@0}: ${%s@1} is nullptr.", TO_STRING(FE::MEMORY_ERROR_1XX::_FATAL_ERROR_NULLPTR), TO_STRING(string_p));
    ...
    _CONSTEXPR20_ fixed_sized_string& replace(const size_type position_p, const size_type count_to_replace_p, const value_type* const string_p) noexcept
    {
        ...
        FE_SUSPECT(((this->m_length + l_input_length) - count_to_replace_p) > this->max_length(), "${%s@0}: failed to replace.", TO_STRING(FE::MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_CAPACITY));
    ...
    _CONSTEXPR20_ fixed_sized_string& replace(const size_type position_p, const size_type count_to_replace_p, std::initializer_list<const CharT>&& initializer_list_p) noexcept
    {
        ...
        FE_SUSPECT(((this->m_length + l_input_size) - count_to_replace_p) > this->max_length(), "${%s@0}: failed to replace.", TO_STRING(FE::MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_CAPACITY));
        FE_SUSPECT(count_to_replace_p == 0, "${%s@0}: ${%s@1} is zero.", TO_STRING(FE::MEMORY_ERROR_1XX::_FATAL_ERROR_INVALID_SIZE), TO_STRING(count_to_replace_p));
    ...
    _CONSTEXPR20_ fixed_sized_string& replace_with_range(const size_type position_p, const size_type count_to_replace_p, const_iterator input_iterator_begin_p, algorithm::string::range input_string_range_p) noexcept
    {
        ...
        FE_SUSPECT(position_p > this->max_length(), "${%s@0}: ${%s@1} must not be greater than ${%s@2}.", TO_STRING(FE::MEMORY_ERROR_1XX::_FATAL_ERROR_INVALID_SIZE), TO_STRING(position_p), TO_STRING(this->max_length()));
