...
namespace concurrency
{
	...
	private:
		tbb::concurrent_vector<mapped_type, allocator_type> m_map;
		tbb::concurrent_bounded_queue<key_type, FE::std_style::cache_aligned_allocator<key_type>> m_fragmented_index_queue;

	public:
		index_map() noexcept : m_map(), m_fragmented_index_queue() {}

		index_map(FE::reserve initial_capacity_p) noexcept : m_map(), m_fragmented_index_queue()
		{
			this->m_map.reserve(initial_capacity_p._value);
			this->m_fragmented_index_queue.set_capacity(initial_capacity_p._value);
		}

		index_map(const index_map& other_p) noexcept : m_map(other_p.m_map), m_fragmented_index_queue(other_p.m_fragmented_index_queue) {}

		index_map(index_map&& rvalue_p) noexcept : m_map(std::move(rvalue_p.m_map)), m_fragmented_index_queue(std::move(rvalue_p.m_fragmented_index_queue)) {}
		...
		{
			this->m_map = other_p.m_map;
			this->m_fragmented_index_queue = other_p.m_fragmented_index_queue;
			return *this;
		}
		_FORCE_INLINE_ index_map& operator=(index_map&& rvalue_p) noexcept
		{
			this->m_map = std::move(rvalue_p.m_map);
			this->m_fragmented_index_queue = std::move(rvalue_p.m_fragmented_index_queue);
			return *this;
		}

		_FORCE_INLINE_ boolean is_empty() noexcept { return (this->m_map.size() - this->m_fragmented_index_queue.size()) == 0; }
		_FORCE_INLINE_ size_type size() noexcept { return this->m_map.size() - this->m_fragmented_index_queue.size(); }
		_FORCE_INLINE_ size_type capacity() noexcept { return this->m_map.capacity(); }
		_FORCE_INLINE_ size_type max_size() noexcept { return FE::max_value<size_type>; }
		...
		{
			this->m_map.clear();
			this->m_fragmented_index_queue.clear();
		}
		...
		{
			key_type l_index_key_value;
			if (this->m_fragmented_index_queue.try_pop(l_index_key_value) == true)
			{
				this->m_map[l_index_key_value] = mapped_type(arguments_p...);
				return l_index_key_value;
			}

			return this->m_map.emplace_back(arguments_p...) - this->m_map.begin();
		}

		_FORCE_INLINE_ void erase(const key_type key_p) noexcept
		{
			this->m_fragmented_index_queue.push(key_p);
		}
		...
		{
			this->m_map.reserve(new_capacity_p);
			this->m_fragmented_index_queue.set_capacity(new_capacity_p);
		}

		_FORCE_INLINE_ void extend(const size_type extra_capacity_p) noexcept
		{
			this->m_map.reserve(this->m_map.capacity() + extra_capacity_p);
			this->m_fragmented_index_queue.set_capacity(this->m_fragmented_index_queue.capacity() + extra_capacity_p);
		}

		_FORCE_INLINE_ void shrink_to_fit() noexcept
		{
			this->m_map.shrink_to_fit();
			this->m_fragmented_index_queue.shrink_to_fit();
		}
	};
