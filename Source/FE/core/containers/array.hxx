...
public:
	...
	{
		FE_CHECK(this->m_array_size == Capacity, "${%s@0}: FE::farray is out of capacity. Unable to push an element to the back of the container.");
	...
	{
		FE_CHECK(this->m_array_size == Capacity, "${%s@0}: FE::farray is out of capacity. Unable to emplace an element to the back of the container.");
	...
	{
		FE_CHECK(this->m_array_size == 0, "${%s@0}: Unable to pop an empty FE::farray.");
...
public:
	...
	{
		FE_CHECK(count_p == 0, "${%s@0}: ${%s@1} cannot be zero.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_INVALID_SIZE), TO_STRING(count_p));
	...
	_CONSTEXPR20_ array(InputIterator first_p, InputIterator last_p) noexcept
	{
		...
		FE_CHECK(l_input_size == 0, "${%s@0}: ${%s@1} cannot be zero.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_INVALID_SIZE), TO_STRING((last_p - first_p)));
	...
	_CONSTEXPR20_ array(std::initializer_list<value_type>&& initializer_list_p) noexcept
	{
		...
		FE_CHECK(l_initializer_list_size == 0, "${%s@0}: ${%s@1} cannot be zero.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_INVALID_SIZE), TO_STRING(initializer_list_p.size()));
	...
	{
		FE_CHECK(count_p == 0, "${%s@0}: ${%s@1} cannot be zero.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_INVALID_SIZE), TO_STRING(count_p));
		FE_CHECK(count_p > this->m_smart_array.capacity(), "${%s@0}: ${%s@1} cannot be greater than the array capacity.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_CAPACITY), TO_STRING(count_p));
	...
	_CONSTEXPR20_ void assign(InputIterator first_p, InputIterator last_p) noexcept
	{
		...
		FE_CHECK(first_p == nullptr, "${%s@0}: ${%s@1} is nullptr", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_NULLPTR), TO_STRING(dest_pointer_p));
		FE_CHECK(last_p == nullptr, "${%s@0}: ${%s@1} is nullptr", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_NULLPTR), TO_STRING(source_pointer_p));
		FE_CHECK(first_p > last_p, "${%s@0}: The begin iterator ${%s@1} must be pointing at the first element of a container.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_ILLEGAL_POSITION), TO_STRING(dest_first_p));
		FE_CHECK(l_input_size == 0, "${%s@0}: ${%s@1} cannot be zero.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_INVALID_SIZE), TO_STRING(l_input_size));
		FE_CHECK(l_input_size > this->m_smart_array.capacity(), "${%s@0}: ${%s@1} cannot be greater than the array capacity.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_CAPACITY), TO_STRING(l_input_size));

	}

	_CONSTEXPR20_ void assign(std::initializer_list<value_type>&& initializer_list_p) noexcept
	{
		FE_CHECK(initializer_list_p.size() == 0, "${%s@0}: ${%s@1} cannot be zero.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_INVALID_SIZE), TO_STRING(initializer_list_p.size()));
		FE_CHECK(initializer_list_p.size() > this->m_smart_array.capacity(), "${%s@0}: ${%s@1} cannot be greater than the array capacity.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_CAPACITY), TO_STRING(initializer_list_p.size()));
	...
	_CONSTEXPR23_ void assign_range(container::range<Container>&& range_p) noexcept
	{
		...
		FE_CHECK(range_p._container.is_valid() == false, "${%s@0}: The container refernce is invalid.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_NULLPTR));
		size_type l_input_size = range_p._end - range_p._begin;
		FE_CHECK(l_input_size > this->m_smart_array.capacity(), "${%s@0}: ${%s@1} cannot be greater than the array capacity.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_CAPACITY), TO_STRING(l_input_size));
		FE_CHECK(range_p._begin > range_p._end, "${%s@0}: The begin index cannot be greater than the end index.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_ILLEGAL_POSITION));
	...
	{
		FE_CHECK(this->m_smart_array.get() == nullptr, "${%s@0}: ${%s@1} was nullptr.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_NULLPTR), TO_STRING(this->m_smart_array.get()));
	...
	{
		FE_CHECK(this->m_smart_array == nullptr, "${%s@0}: FE::array is null.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_NULLPTR));
		FE_CHECK(position_p >= this->size(), "${%s@0}: Unable to access an element that does not belong to this container. ${%s@1} was ${%lu@2}", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_RANGE), TO_STRING(position_p), &position_p);
	...
	{
		FE_CHECK(this->m_array_size == 0, "${%s@0}: Unable to shrink_to_fit() an empty string instance.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_INVALID_SIZE));
		FE_CHECK(this->m_smart_array == nullptr, "${%s@0}: ${%s@1} was nullptr.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_NULLPTR), TO_STRING(this->m_smart_array));
	...
	{
		FE_CHECK(this->m_smart_array == nullptr, "${%s@0}: ${%s@1} was nullptr.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_NULLPTR), TO_STRING(this->m_smart_array.get()));
	...
	{
		FE_CHECK(position_p >= this->m_smart_array.capacity(), "${%s@0}: ${%s@1} exceeds the array capacity. ${%s@1} was ${%lu@2}, and ${%s@3} was ${%lu@4}", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_CAPACITY), TO_STRING(position_p), &position_p, TO_STRING(this->m_smart_array.capacity()), &FE::buffer<size_type>::set_and_get(this->m_smart_array.capacity()));

	}

	_CONSTEXPR20_ iterator erase(index_t position_p, size_type count_p = 1) noexcept
	{
		FE_CHECK(position_p + count_p >= this->m_smart_array.capacity(), "${%s@0}: ${%s@1} exceeds the array capacity. ${%s@1} was ${%lu@2}, and ${%s@3} was ${%lu@4}", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_OUT_OF_CAPACITY), TO_STRING(position_p + count_p), &FE::buffer<size_type>::set_and_get(position_p + count_p), TO_STRING(this->m_smart_array.capacity()), &FE::buffer<size_type>::set_and_get(this->m_smart_array.capacity()));
		FE_CHECK(count_p == 0, "${%s@0}: ${%s@1} cannot be zero.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_INVALID_SIZE), TO_STRING(count_p));
		FE_CHECK(position_p + count_p >= this->m_last_index, "Assertion Failure: It is not allowed to erase() the back of an array. Use pop_back() instead.");
	...
	{
		FE_CHECK(this->m_last_index > this->m_smart_array.capacity(), "${%s@0}: Unable to pop the back of an empty array.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_ACCESS_VIOLATION));
	...
	{
		FE_CHECK(this->m_array_size == 0, "${%s@0}: Unable to pop the back of an empty array.", TO_STRING(MEMORY_ERROR_1XX::_FATAL_ERROR_ACCESS_VIOLATION));
