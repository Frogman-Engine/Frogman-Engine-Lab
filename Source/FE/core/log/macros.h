...
{ \
	...
	::FE::log::__FE_ASSERT_IMPLEMENTATION(::FE::log::buffered_string_formatter({ __VA_ARGS__ }), __FE_ASSERT_SOURCE_DIR__, __FE_ASSERT_FUNCTION_NAME__, __FE_ASSERT_CODE_LINE__); \
	assert(!(expression)); \
}
#else
#define FE_ASSERT(expression, ...)
